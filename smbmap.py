...
class SMBMap():
    ...
    def list_shares(self, verbose=False):
    ...
    def list_path_recursive(self, share, pwd, wildcard, pathList, pattern, verbose):
                        ...
                        try:
                            ...
                            if smbItem.is_directory() > 0 and filename != '.' and filename != '..':
                                ...
                                if len(pathList[subPath]) > 2:
                                    self.list_path_recursive(share, '%s/%s' % (pwd, filename), wildcard, pathList, pattern, verbose)
    ...
    def list_path(self, share, path, pattern, verbose=False):
                ...
                if error == 0 and (len(set(sys.argv).intersection(['-r','-R'])) == 1):
                    ...
                    if '-r' in sys.argv:
                        if lsshare and lspath:
                            ...
                            dirList = mysmb.list_path(lsshare, lspath, pattern, verbose)
                            sys.exit()
                        else:
                            if '-A' in sys.argv:
                                print '\t[+] Starting search for files matching \'%s\' on share %s.' % (pattern, share)
                            dirList = mysmb.list_path(share, path, pattern, verbose)

                    elif '-R' in sys.argv:
                        if lsshare and lspath:
                            if '-A' in sys.argv:
                                print '\t[+] Starting search for files matching \'%s\' on share %s.' % (pattern, lsshare)
                            dirList = mysmb.list_path_recursive(lsshare, lspath, '*', pathList, pattern, verbose)
                            sys.exit()
                        else:
                            if '-A' in sys.argv:
                                print '\t[+] Starting search for files matching \'%s\' on share %s.' % (pattern, share)
                            dirList = mysmb.list_path_recursive(share, path, '*', pathList, pattern, verbose)
